...
* Noah propose une modification du classement : Ajoutant de nouvelle façon de classer les joueurs ( On conserve le classement par rang + rajout de tri/classement par colonnes [ Classement par Stockage / Production / Ecu ])
	...
	- Idées reprises de la maquette de Noah : 
		...
		- Dans le Poulailler : Il faut une généralisation des oeufs. Il nous faut un total d'oeuf de toutes les poules et non pas par poule ! Il faudra aussi faire attention à la gestion des poussins, leur décès en fonction de leur âge. Le bouton nourir a fait débat, Faut-il un bouton pour nourir toutes les poules d'un seul coup ou un bouton pour nourir les poules individuellement ? QUe se passe-t-il si le joueur n'a pas assez de ressource pour toutes les nourir ? Un bouton général rajouterai du travail pour l'équipe BackEnd.
		
		- Rajout d'un pop-up, notif à chaque début de journée pour avoir un récap de ce qu'il s'est passé la journée précédente.
...
* Séparation en plusieurs groupes de 2 pour chaque pages. 
	...
	- Besoin des spécifications technique par les noms de variables par le FrontEnd pour le BackEnd.

	- Pages HTML nécéssaires :
		- 6 / 7 Pages à faire

	- 1 personne en stand-by chez FrontEnd : Peut aider BackEnd si besoin.
...
- Reflexion sur la conception du projet :
	- Mewenn ( Chef de l'équipe BackEnd ) Propose de commencer par la BDD au vu du TD de ce matin
		...
		Idée d'Aubry : Faire un maximum de fonction dans la BDD en Postgresql 
		Idée de Savio : Il y aura une couche de structure qui s'occupera des requête ==> BDD seulement requête et table. - Architecture API Rest : Representative rest transfert ( En java ) 
			- S'organise en couches ( Model - Repo - Service - Controller )
				Model      :
				Repo       : Requête personalisé pour SpringBoot
				Service	   : Action
				Controller : Vérifié les URL qu'on envoi au serveur ( Requête ) Besoin de géré la récupération de donné .
			- C'est pour ça qu'on devrait réalisé les tables et les vues ect... le reste sera géré par le REST. ( " On est là pour réalisé un projet en Java et non pas en Postgresql." ).

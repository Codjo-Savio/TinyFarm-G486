...
public class MarketControllerTest extends AuthenticatedControllerTestSupport {
    ...
    void shouldReturnMarketByUserId() throws Exception {
        mockMvc.perform(get("/api/market/id/1").with(authenticated()))
                .andExpect(status().isOk());
    }

    @Test
    void shouldReturnMarketByProductId() throws Exception {
        mockMvc.perform(get("/api/market/product/" + productId).with(authenticated()))
                .andExpect(status().isOk());
    }

    @Test
    void shouldReturnMarketByPrice() throws Exception {
        mockMvc.perform(get("/api/market/price/13.0").with(authenticated()))
                .andExpect(status().isOk());
    }

    @Test
    void shouldReturnMarketByQuantity() throws Exception {
        mockMvc.perform(get("/api/market/quantity/100").with(authenticated()))
                .andExpect(status().isOk());
    }

    @Test
    void marketShouldNotBeFoundByUserId() throws Exception {
        mockMvc.perform(get("/api/market/id/999").with(authenticated()))
                .andExpect(status().isNotFound());
    }

    @Test
    void marketShouldNotBeFoundByProductId() throws Exception {
        mockMvc.perform(get("/api/market/product/999").with(authenticated()))
                .andExpect(status().isNotFound());
    }
    ...
    void shouldUpdateMarket() throws Exception {
        ...
        updatedMarket.setProductId(productId);
    ...
    void shouldDeleteMarketByUserIdAndProductId() throws Exception {
        mockMvc.perform(delete("/api/market/1/" + productId).with(authenticated()))
                .andExpect(status().isNoContent());
    }

    @Test
    void shouldDeleteMarketByUserId() throws Exception {
        mockMvc.perform(delete("/api/market/id/1").with(authenticated()))
                .andExpect(status().isNoContent());
    }
    ...
    void shouldBuyFromMarket() throws Exception {
        ...
        // Création du produit utilisé pour l'achat
        Product product = new Product();
        product.setDescription("Produit test buy");
        Product saved = productService.create(product);
        Long buyProductId = saved.getId();

        Stock sellerStock = new Stock();
        sellerStock.setId(new StockId(seller.getId(), buyProductId));
        ...
        listing.setProductId(buyProductId);
        ...
        request.put("productId", buyProductId);
